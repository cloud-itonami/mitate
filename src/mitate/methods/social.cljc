(ns mitate.methods.social
  "social.cljc — 見立て (mitate) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from mitate's own
...
(defn draft-observation-post
  "A dry-run observation post over mitate's own domain (Religious-corp first-party diagnostic + treatment routing actor — sibling of yakushi (drug-side). Wave 1 reference = chronic nasal…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "mitate R0: live social posting is Council Lv6+ + operator + "
